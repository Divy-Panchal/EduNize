...
export function Timetable() {
  ...
  const getClassForSlot = (dayIndex: number, time: string) => {
    // Match classes that start within this hour
    const hourPrefix = time.split(':')[0]; // e.g., "09" from "09:00"
    return classes.find(cls => cls.day === dayIndex && cls.time.startsWith(hourPrefix));
  };
  ...
  const handleAddClass = (e: React.FormEvent) => {
    ...
    // Check for conflicts
    const newStart = parseInt(newClass.time.split(':')[0]);
    const newEnd = newStart + newClass.duration;

    const hasConflict = classes.some(cls => {
      if (cls.day !== newClass.day) return false;
      const clsStart = parseInt(cls.time.split(':')[0]);
      const clsEnd = clsStart + cls.duration;
      return newStart < clsEnd && clsStart < newEnd;
    });

    if (hasConflict) {
      toast.error('Class clashes with an existing slot!');
      return;
    }
    ...
    <div className="h-full flex flex-col overflow-y-auto">
      ...
      <div className="flex-1 p-4 md:p-6 pt-2 pb-24 md:pb-6 scroll-smooth">
        ...
        <motion.div
          ...
          className={`${themeConfig.card} p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 mb-4`}
        >
        ...
        {viewMode === 'month' ? (
          ...
          <>
            ...
            {/* Timetable Grid Headers */}
            <div className="grid grid-cols-8 gap-1 md:gap-2 mb-2">
              {/* Time column header */}
              <div className={`${themeConfig.card} p-1 md:p-3 text-center rounded-lg border dark:border-gray-700`}>
              ...
              {weekDays.map((day, index) => (
                <motion.div
                  ...
                  className={`${themeConfig.card} p-1 md:p-3 text-center rounded-lg border dark:border-gray-700`}
                >
                  <div className={`font-medium ${themeConfig.text} text-xs md:text-sm`}>
                    {format(day, 'EEE')}
                  </div>
              ...
              <form onSubmit={handleAddClass} className="space-y-4">
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <motion.button
                    ...
                    type="button"
                    onClick={() => setShowAddModal(false)}
                    className={`flex-1 ${themeConfig.card} border dark:border-gray-600 ${themeConfig.text} py-2 md:py-3 rounded-lg font-medium transition-colors duration-200 text-sm md:text-base`}
                  >
                    Cancel
                  </motion.button>
                </div>
              ...
              <div className="space-y-4">
                ...
                {selectedTask.description && (
                  <div>
                    ...
                    <p className={`font-medium ${themeConfig.text}`}>{selectedTask.description}</p>
                  </div>
                )}
                ...
                {selectedTask.dueDate && (
                  <div>
                    ...
                    <p className={`font-medium ${themeConfig.text}`}>{format(new Date(selectedTask.dueDate), 'MMM dd, yyyy')}</p>
                  </div>
                )}
              </div>
